# Softpowers Release Notes

## v5.0.7 (2026-03-31)

### GitHub Copilot CLI Support

- **SessionStart context injection**: Copilot CLI v1.0.11 added support for `additionalContext` in sessionStart hook output. The session-start hook now detects the `COPILOT_CLI` environment variable and emits the SDK-standard `{ "additionalContext": "..." }` format, giving Copilot CLI users the full softpowers bootstrap at session start. (Original fix by @culinablaz in PR #910)
- **Tool mapping**: added `references/copilot-tools.md` with the full Claude Code to Copilot CLI tool equivalence table
- **Skill and README updates**: added Copilot CLI to the `using-softpowers` skill's platform instructions and README installation section

### OpenCode Fixes

- **Skills path consistency**: the bootstrap text no longer advertises a misleading `configDir/skills/softpowers/` path that didn't match the runtime path. The agent should use the native `skill` tool, not navigate to files by path. Tests now use consistent paths derived from a single source of truth. (#847, #916)
...
- **Added `package.json`** so OpenCode can install softpowers as an npm package from git.
...
- `GEMINI.md` @imports `using-softpowers` skill and tool mapping table at session start
- Gemini CLI tool mapping reference (`skills/using-softpowers/references/gemini-tools.md`): translates Claude Code tool names (Read, Write, Edit, Bash, etc.) to Gemini CLI equivalents (read_file, write_file, replace, etc.)
...
- Specs (brainstorming output) now save to `docs/softpowers/specs/YYYY-MM-DD-<topic>-design.md`
- Plans (writing-plans output) now save to `docs/softpowers/plans/YYYY-MM-DD-<feature-name>.md`
...
Writing-plans no longer offers a choice between subagent-driven and executing-plans. On harnesses with subagent support (Claude Code, Codex), subagent-driven-development is required. Executing-plans is reserved for harnesses without subagent capability, and now tells the user that Softpowers works better on a subagent-capable platform.
...
- `lib/brainstorm-server/`: WebSocket server with browser helper library, session management scripts, and dark/light themed frame template ("Softpowers Brainstorming" with GitHub link)
...
- Design spec and implementation plan in `docs/softpowers/`
...
Added explicit priority ordering to using-softpowers:

1. User's explicit instructions (CLAUDE.md, AGENTS.md, direct requests): highest priority
2. Softpowers skills: override default system behavior
...
Added `<SUBAGENT-STOP>` block to using-softpowers. Subagents dispatched for specific tasks now skip the skill instead of activating the 1% rule and invoking full skill workflows.
...
This fix should dramatically improve softpowers skills compliance and should reduce the chances of Claude entering its native plan mode unintentionally.
...
**Using-softpowers workflow graph intercepts EnterPlanMode**
...
Changed `async: true` to `async: false` in hooks.json. When async, the hook could fail to complete before the model's first turn, meaning using-softpowers instructions weren't in context for the first message.
...
The `softpowers-codex` bootstrap CLI, Windows `.cmd` wrapper, and related bootstrap content file have been removed. Codex now uses native skill discovery via `~/.agents/skills/softpowers/` symlink, so the old `use_skill`/`find_skills` CLI tools are no longer needed.
...
The synchronous SessionStart hook blocked the TUI from entering raw mode on Windows, freezing all keyboard input. Running the hook async prevents the freeze while still injecting softpowers context.
...
- Windows doesn't respect shebangs, so directly invoking the extensionless `softpowers-codex` script triggered an "Open with" dialog. All invocations now prefixed with `node`.
...
Softpowers for OpenCode now uses OpenCode's native `skill` tool instead of custom `use_skill`/`find_skills` tools. This is a cleaner integration that works with OpenCode's built-in skill discovery.

**Migration required:** Skills must be symlinked to `~/.config/opencode/skills/softpowers/` (see updated installation docs).
...
**Strengthened using-softpowers skill for explicit skill requests**
...
The underlying skills (`softpowers:brainstorming`, `softpowers:executing-plans`, `softpowers:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-softpowers` skill now explicitly states that the Skill tool loads skill content directly-no need to read files.

- Added "How to Access Skills" section to `using-softpowers`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `softpowers:brainstorming`)
...
**Skill priority in using-softpowers**
...
- **Rationalizations table** in using-softpowers - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-softpowers is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > softpowers
...
- Optimized softpowers bootstrap to eliminate redundant skill execution. The `using-softpowers` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-softpowers` manually despite already having the content from session start.
...
- Added unified `softpowers-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `softpowers:skill-name` for softpowers skills, `skill-name` for personal
- Personal skills override softpowers skills when names match
...
- Updated terminology: "Softpowers skills" instead of "Core skills"

### Files Added
- `.codex/INSTALL.md` - Installation guide for Codex users
- `.codex/softpowers-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `.codex/softpowers-codex` - Unified Node.js executable with all functionality

**Note:** Codex support is experimental. The integration provides core softpowers functionality but may require refinement based on user feedback.

## v3.2.3 (2025-10-23)

### Improvements

**Updated using-softpowers skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-softpowers/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-softpowers skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-softpowers/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `softpowers:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `softpowers:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `softpowers:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `softpowers:code-reviewer`
...
- All internal skill references now use `softpowers:` namespace prefix
- Updated format: `softpowers:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/softpowers:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
# Softpowers v2.0.0 Release Notes

## Overview

Softpowers v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([obra/superpowers-skills](https://github.com/obra/superpowers-skills)). This transforms softpowers from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
- **First install:** Plugin automatically clones skills to `~/.config/softpowers/skills/`
...
1. Your old `~/.config/softpowers/.git` will be backed up to `~/.config/softpowers/.git.bak`
2. Old skills will be backed up to `~/.config/softpowers/skills.bak`
3. Fresh clone of obra/superpowers-skills will be created at `~/.config/softpowers/skills/`

### Removed Features

- **Personal softpowers overlay system** - Replaced with git branch workflow
- **setup-personal-softpowers hook** - Replaced by initialize-skills.sh
...
- `SOFTPOWERS_SKILLS_ROOT` set to `~/.config/softpowers/skills`
...
- Removed obsolete setup-personal-softpowers call from session-start
...
- Prominent link to softpowers-skills repo
...
- `hooks/setup-personal-softpowers.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/softpowers/skills
- `commands/brainstorm.md` - Updated paths to SOFTPOWERS_SKILLS_ROOT
- `commands/write-plan.md` - Updated paths to SOFTPOWERS_SKILLS_ROOT
- `commands/execute-plan.md` - Updated paths to SOFTPOWERS_SKILLS_ROOT
...
- PR #2: Personal softpowers overlay system (later replaced)
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/softpowers/skills ~/softpowers-skills-backup
   ```
