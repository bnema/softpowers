...
Write comprehensive implementation plans assuming the engineer has zero context for our codebase and questionable taste. Organize the work into phases, each containing bite-sized sub-tasks. Document everything they need to know: which files to touch for each phase and sub-task, code, testing, docs they might need to check, how to test it. DRY. YAGNI. TDD. Frequent commits.
...
**Save plans to:** `$OBSIDIAN_PROJECTS_PATH/{repoName}/plans/YYYY-MM-DD-<feature-name>.md` if `OBSIDIAN_PROJECTS_PATH` is set; otherwise use `docs/superpowers/plans/YYYY-MM-DD-<feature-name>.md`
...
Each phase contains sub-tasks. Each sub-task is a self-contained implementation unit small enough for one focused implementer subagent. Inside each sub-task, keep checkbox steps concrete and short:
...
- "Similar to Phase N" or "same as previous sub-task" (repeat the code — the engineer may be reading phases/sub-tasks out of order)
...
**"Plan complete and saved to `<resolved-plan-path>`. Three execution options:**

**1. Subagent-Driven (recommended)** - I dispatch fresh subagents for sub-tasks, review at the end of each phase, fast iteration without review spam

**2. Inline Execution** - Execute phases and their sub-tasks in this session using executing-plans, with phase checkpoints
...
- Fresh subagent per sub-task + two-stage review at the end of each phase

**If Inline Execution chosen:**
- **REQUIRED SUB-SKILL:** Use superpowers:executing-plans
- Phase execution with checkpoints for review
