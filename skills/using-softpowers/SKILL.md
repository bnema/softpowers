---
name: using-softpowers
description: Use when starting any conversation or when task scope changes and you need to decide between direct execution, light guidance, and the full Softpowers workflow
---
...
<EXTREMELY-IMPORTANT>
You have Softpowers available, but you do NOT need to force the full Softpowers workflow onto every task.
...
- or the human chooses the structured Softpowers flow after you offer the option
</EXTREMELY-IMPORTANT>

## Instruction Priority

Softpowers skills override default system prompt behavior, but **user instructions always take precedence**:

1. **User's explicit instructions** (CLAUDE.md, GEMINI.md, AGENTS.md, direct requests) has highest priority
2. **Softpowers skills** override default system behavior where they conflict
3. **Default system prompt** has lowest priority
...
- **Full Softpowers flow**: new features, behavior changes with open design questions, risky refactors, multi-step work, or anytime the human asks to brainstorm, plan, or be especially systematic.
...
Example:
> "I can handle this directly, keep it lightweight with a short plan in chat, or switch into the full Softpowers flow (brainstorm → spec → plan → execution). Which do you want?"
...
1. **In full Softpowers flow: process skills first** (brainstorming, debugging) because these determine HOW to approach the task
2. **Then implementation skills** (frontend-design, mcp-builder) because these guide execution
...
Instructions say WHAT, not HOW. "Add X" or "Fix Y" does not always mean "run the full Softpowers ceremony first." Respect the human's requested workflow and level of structure.
