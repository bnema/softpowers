...
| Dispatch a subagent (`Subagent (general-purpose):` template) | `spawn_agent` (see [Subagent dispatch requires multi-agent support](#subagent-dispatch-requires-multi-agent-support) and [Named agent dispatch](#named-agent-dispatch)) |
...
Claude Code skills reference named agent types like `softpowers:code-reviewer`.
...
|-------------------|------------------|
| `Task tool (softpowers:code-reviewer)` | `spawn_agent(agent_type="worker", message=...)` with `code-reviewer.md` content |
