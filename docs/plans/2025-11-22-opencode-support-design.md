...
Add full softpowers support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port softpowers to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/softpowers-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/softpowers.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/softpowers/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/softpowers-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "softpowers:brainstorming")')
  }),
...
1. **Inject using-softpowers content**
   - Full content of the using-softpowers skill
...
```javascript
// .opencode/plugin/softpowers.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const SoftpowersPlugin = async ({ client, directory, $ }) => {
  const softpowersDir = path.join(process.env.HOME, '.config/opencode/softpowers');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingSoftpowers = await readSkill('using-softpowers');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingSoftpowers}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── softpowers-codex        # UPDATED: Use skills-core
│   ├── softpowers-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── softpowers.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/softpowers-codex`
...
2. Update `.codex/softpowers-codex` to use shared core
...
1. Create `.opencode/plugin/softpowers.js`
