...
> **For agentic workers:** REQUIRED SUB-SKILL: Use softpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add full softpowers support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/softpowers-codex` (lines 40-74)
...
- Reference: `.codex/softpowers-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'softpowers' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/softpowers-codex` (lines 212-280)
 ...
 * (personal skills override softpowers skills).
 *
 * @param {string} skillName - Name like "softpowers:brainstorming" or "my-skill"
 * @param {string} softpowersDir - Path to softpowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, softpowersDir, personalDir) {
    // Strip softpowers: prefix if present
    const forceSoftpowers = skillName.startsWith('softpowers:');
    const actualSkillName = forceSoftpowers ? skillName.replace(/^softpowers:/, '') : skillName;

    // Try personal skills first (unless explicitly softpowers:)
    if (!forceSoftpowers && personalDir) {
    ...
    // Try softpowers skills
    if (softpowersDir) {
        const softpowersPath = path.join(softpowersDir, actualSkillName);
        const softpowersSkillFile = path.join(softpowersPath, 'SKILL.md');
        if (fs.existsSync(softpowersSkillFile)) {
            return {
                skillFile: softpowersSkillFile,
                sourceType: 'softpowers',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/softpowers-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/softpowers-codex` (add import at top)
...
Run: `node -c .codex/softpowers-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/softpowers-codex
...
**Files:**
- Modify: `.codex/softpowers-codex` (lines 40-74)
...
Run: `.codex/softpowers-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/softpowers-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/softpowers-codex` (lines 16-38, approximately)
...
Run: `.codex/softpowers-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/softpowers.js`
...
/**
 * Softpowers plugin for OpenCode.ai
 *
...
const softpowersSkillsDir = path.join(homeDir, '.config/opencode/softpowers/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const SoftpowersPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/softpowers.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/softpowers.js
...
**Files:**
- Modify: `.opencode/plugin/softpowers.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "softpowers:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > softpowers)
...
Run: `node -c .opencode/plugin/softpowers.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the softpowers and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const softpowersSkills = skillsCore.findSkillsInDir(
            softpowersSkillsDir,
            'softpowers',
            3
          );
          ...
          const allSkills = [...personalSkills, ...softpowersSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install softpowers skills to ~/.config/opencode/softpowers/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'softpowers:';
...
```javascript
    'session.started': async () => {
      // Read using-softpowers skill content
      const usingSoftpowersPath = skillsCore.resolveSkillPath(
        'using-softpowers',
        softpowersSkillsDir,
        personalSkillsDir
      );

      let usingSoftpowersContent = '';
      if (usingSoftpowersPath) {
        const fullContent = fs.readFileSync(usingSoftpowersPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingSoftpowersContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Softpowers skills: \`softpowers:skill-name\` (from ~/.config/opencode/softpowers/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/softpowers')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/softpowers && git pull` to update softpowers.' :
        '';
...
**Below is the full content of your 'softpowers:using-softpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingSoftpowersContent}
...
### 1. Install Softpowers Skills

```bash
# Clone softpowers skills to OpenCode config directory
mkdir -p ~/.config/opencode/softpowers
git clone https://github.com/bnema/softpowers.git ~/.config/opencode/softpowers
```

### 2. Install the Plugin

The plugin is included in the softpowers repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/softpowers/.opencode/plugin/softpowers.js`
...
ln -s ~/.config/opencode/softpowers/.opencode/plugin/softpowers.js .opencode/plugin/softpowers.js
```
...
```
use use_skill tool with skill_name: "softpowers:brainstorming"
```
...
Personal skills override softpowers skills with the same name.

## Updating

```bash
cd ~/.config/opencode/softpowers
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/softpowers/.opencode/plugin/softpowers.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/softpowers/skills`
...
- Documentation: https://github.com/bnema/softpowers
```
...
Softpowers works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/softpowers-codex use-skill softpowers:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/softpowers-codex bootstrap | head -30`
...
ls -l .opencode/plugin/softpowers.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/softpowers.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/softpowers-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/softpowers-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/softpowers.js` created with tools and hooks
