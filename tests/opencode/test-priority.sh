...
# Verifies that skills are resolved with correct priority: project > personal > softpowers
...
# 1. Create in softpowers location (lowest priority)
mkdir -p "$SOFTPOWERS_SKILLS_DIR/priority-test"
cat > "$SOFTPOWERS_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Softpowers version of priority test skill
---
# Priority Test Skill (Softpowers Version)

This is the SOFTPOWERS version of the priority test skill.

PRIORITY_MARKER_SOFTPOWERS_VERSION
EOF
...
if [ -f "$SOFTPOWERS_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Softpowers version exists"
else
    echo "  [FAIL] Softpowers version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides softpowers
echo ""
echo "Test 2: Testing personal > softpowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides softpowers)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_SOFTPOWERS_VERSION"; then
    echo "  [FAIL] Softpowers version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|softpowers" | head -10
fi

# Test 3: Test that project overrides both personal and softpowers
echo ""
echo "Test 3: Testing project > personal > softpowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Softpowers version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit softpowers: prefix bypasses priority
echo ""
echo "Test 4: Testing softpowers: prefix forces softpowers version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load softpowers:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_SOFTPOWERS_VERSION"; then
    echo "  [PASS] softpowers: prefix correctly forces softpowers version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] softpowers: prefix did not force softpowers version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
